...
namespace GalleryStoreManagerDotNetStandard
{
    class Program
    {
        ...
        {
            var locationOfAssembly = typeof(Program).Assembly.Location;
            var currentPath = Path.GetDirectoryName(locationOfAssembly);

            var getPathForTests = Directory.GetDirectories(currentPath + "/../../../.././");
            var testPath = getPathForTests.SingleOrDefault(x => x.ToLower().Contains("testsdirectory"));
